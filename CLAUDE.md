...
## 自分の業務に合わせる（「AIエージェントをカスタマイズ」）

受講者が「自分の業務用にしたい」「シートのとおりに作りたい」と言ったら，`worksheets/` の**いちばん新しいシート**を読んで始める．シートが無ければ，先に分解シートを埋めることを勧める．

- **順番は 指示書 → 道具 → トリガー．** まず計画（何を足すか 3 行）を出して受講者に確認してから，1 つずつ進める
...
- **会社の本番データにつなぐときは読み取りから．** 書き込み・送信をする道具は，実行前に受講者へ確認を入れる作りにする．練習用で動いてから本番へ
- モデルは小さい方（Qwen3-30b）で試し，コンテキスト超過や精度不足のときだけ大きい方に替える．替えるときは費用差を一言伝える
- Slack の受け口（`src/index.ts` の署名検証・イベント処理）は触らない
